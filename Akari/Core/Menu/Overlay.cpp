...
void Overlay::OnShutdown()
{
	if (UpdateInfoThreadId != SYS_PPU_THREAD_ID_INVALID)
	{
		...
		Timers::Sleep(g_Config.overlay.refreshDelay + 500);
...
void Overlay::DrawOverlay()
{
	...
	{
		wchar_t tmp[16];
		stdc::swprintf(tmp, 16, L"FPS: %%.%if\n", g_Config.overlay.fpsPrecision);

		stdc::swprintf(buffer, 50, tmp, m_FPS);
		overlayText += buffer;
	}

	if (g_Config.overlay.drawScreenRes)
	{
		stdc::swprintf(buffer, 50, L"Screen resolution: %ix%i\n", paf_F476E8AA(), paf_AC984A12());
		overlayText += buffer;
	}

	if (g_Config.overlay.drawMemory)
	{
		stdc::swprintf(buffer, 50, L"RAM: %.1f%% %i / %i KB\n", m_MemoryUsage.percent, m_MemoryUsage.used, m_MemoryUsage.total);
		overlayText += buffer;
	}

	if (g_Config.overlay.drawCPUTemp)
	{
		stdc::swprintf(buffer, 50, L"CPU: %.1f", m_CPUTemp);
		overlayText += buffer + GetTemperatureSymbol() + (g_Config.overlay.drawGPUTemp ? L" / " : L"\n");
	}

	if (g_Config.overlay.drawGPUTemp)
	{
		stdc::swprintf(buffer, 50, L"GPU: %.1f", m_GPUTemp);
		overlayText += buffer + GetTemperatureSymbol() + L"\n";
	}

	if (g_Config.overlay.drawFanSpeed)
	{
		stdc::swprintf(buffer, 50, L"Fan speed: %.1f%%\n", m_FanSpeed);
		overlayText += buffer;
	}
