...
namespace Timers
{
	...
	ValueTimer::ValueTimer(float* from, float to, unsigned int duration, unsigned int startDelay)
	...
	bool ChangeOvertime::Add(float* from, float to, unsigned int duration, unsigned int startDelay, bool ignoreCheck)
	{
	...
	bool ChangeOvertime::Add(vec2* from, vec2 to, unsigned int duration, unsigned int startDelay, bool ignoreCheck)
	{
		for (int i = 0; i < 2; i++)
	...
	bool ChangeOvertime::Add(vec3* from, vec3 to, unsigned int duration, unsigned int startDelay, bool ignoreCheck)
	{
		for (int i = 0; i < 3; i++)
			if (!Add(&from->v[i], to.v[i], duration, startDelay, ignoreCheck))
				return false;
	...
	bool ChangeOvertime::Add(vec4* from, vec4 to, unsigned int duration, unsigned int startDelay, bool ignoreCheck)
	{
		for (int i = 0; i < 4; i++)
			if (!Add(&from->v[i], to.v[i], duration, startDelay, ignoreCheck))
				return false;
