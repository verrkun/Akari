...
namespace Memory
{
	...
	size_t Detour::JumpWithOptions(void* destination, const void* branchTarget, bool linked, bool preserveRegister, uint32_t branchOptions, uint8_t conditionRegisterBit, uint8_t registerIndex)
	{
		...
		if (destination)
			Memory::WriteProcessMemory(sys_process_getpid(), (uint32_t)destination, BranchAsm, BranchAsmSize);
	...
	size_t Detour::RelocateBranch(uint32_t* destination, uint32_t* source)
	{
		...
		{
			Memory::WriteProcessMemory(sys_process_getpid(), (uint32_t)destination, &Instruction, sizeof(Instruction));
	...
	void Detour::Hook(uint32_t fnAddress, uint32_t fnCallback, uint32_t tocOverride)
	{
		...
		Memory::WriteProcessMemory(sys_process_getpid(), (uint32_t)m_OriginalInstructions, m_HookAddress, HookSize);
	...
	bool Detour::UnHook()
	{
		...
		{
			Memory::WriteProcessMemory(sys_process_getpid(), (uint32_t)m_HookAddress, m_OriginalInstructions, m_OriginalLength);
